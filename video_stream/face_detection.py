...
classes = ['man', 'woman']
age_net = cv2.dnn.readNetFromCaffe('deploy_age.prototxt.txt', 'age_net.caffemodel')
...
while webcam.isOpened():
    ...
    for idx, f in enumerate(face):
       ...
       # idx = np.argmax(conf)
      #  label = classes[idx]
        blob = cv2.dnn.blobFromImage(face_detect, 1, (244,244), MODEL_MEAN_VALUES, swapRB=True)
       ...
       # cv2.imshow('frame', frame)
        ...
        cv2.putText(frame, label, (startX, Y), cv2.FONT_HERSHEY_SIMPLEX,
                    0.7, (0, 255, 0), 2)
        cv2.putText(frame,age,(endX, Z), cv2.FONT_HERSHEY_SIMPLEX,
                                    0.7, color=(0, 255,0))
